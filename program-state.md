...
| infergate impl | IG-T002+T003+T004 rebuild (contracts v0.1.0 now on disk) | rebuilt (commits `0e4483b`,`26b2ad5`,`f889896`; agent-reported: conformance 91/0/5 skipped-by-design vs 41 fixtures; 100 concurrent streams 0 integrity violations; 3-pt cancel queued 462µs / pre-token 845µs / mid-stream p95 1.63ms; goleak package-wide; reload 4279 req/0 fail across 145 publishes, publish max 0.303ms; drain e2e green; D4 vendored pin sha256-verified, D5 sse-ref kit gap). **G2 fresh-context verifier RUNNING iter 17** — gate not marked until verifier passes |
| inferbench cont. | IB-T003 workloads + IB-T004 client calibration (gateway+mock exist again) | agent dispatched iter 17 |
